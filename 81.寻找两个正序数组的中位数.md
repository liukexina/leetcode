...
var findMedianSortedArrays = function(nums1, nums2) {
    ...
    var mid1, mid2, pre;
    if (length % 2 === 0) {
        mid1 = Math.floor((length - 1) / 2);
        mid2 = Math.ceil((length - 1) / 2);
    } else {
        mid1 = mid2 = Math.floor((length - 1) / 2);
    }
    let index = -1;
    while (nums1.length > 0 || nums2.length > 0) {
        ...
        index++;
        if (mid1 === index) pre = item;
        if (mid2 === index) return (pre + item) / 2
    }
};
```

## 二分查找
思路：https://leetcode-cn.com/problems/median-of-two-sorted-arrays/solution/xiang-xi-tong-su-de-si-lu-fen-xi-duo-jie-fa-by-w-2/
